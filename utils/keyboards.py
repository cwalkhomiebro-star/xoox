...
def get_buy_stars_button(lang: str, user_id: int, text: str = None, is_crypto: bool = False):
    ...
    if CASHIER_BOT_USERNAME and CASHIER_BOT_USERNAME != "YourCashierBotUsername" and user_id:
        path = "pay_" + str(user_id) + ("_crypto" if is_crypto else "_stars")
        pricing_url = f"tg://resolve?domain={CASHIER_BOT_USERNAME}&start={path}"
...
def get_main_menu_markup(lang: str = "en", user_id: int = None):
    ...
    pricing_btn = get_buy_stars_button(lang, user_id, is_crypto=False)
